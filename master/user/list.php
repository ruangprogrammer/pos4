<?php 
if (isset($_GET['hapus'])) {
  $cekUser = mysqli_query($mysqli,"SELECT * FROM user WHERE user_id = '".$_GET['hapus']."'");
  $dataUser = mysqli_fetch_array($cekUser);
  if(($dataUser['user_level'] == 'Admin') || ($dataUser['user_level'] == 'owner')){
   echo "<script> alert('Maaf User Level Admin tidak bisa dihapus'); location.href='index.php?hal=master/user/list' </script>";exit;
 }else{
  $queryHapus = mysqli_query($mysqli,"DELETE FROM user where user_id = '".$_GET['hapus']."'");
        ...
        <div class="adv-table editable-table ">
          ...
          <table class="table table-striped table-hover table-bordered" id="editable-sample">
            ...
            <tbody>
             <?php 
             $no = 0;
             $queryUsers = mysqli_query($mysqli,"SELECT * FROM user ORDER BY user_id DESC");
